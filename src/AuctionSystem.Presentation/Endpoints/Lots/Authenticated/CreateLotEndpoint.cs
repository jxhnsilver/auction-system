...
namespace AuctionSystem.Presentation.Endpoints.Lots.Authenticated
{
    public sealed class CreateLotEndpoint : IEndpoint
    {
        ...
        {
            app.MapPost("api/lots", (Request request, ISender sender, CancellationToken cancellationToken)
        ...
        private static async Task<IResult> Create(Request request, ISender sender, CancellationToken cancellationToken)
        {
            ...
            if (result.IsSuccess)
                return Results.Created($"/api/lots/{result.Value}", new Response(result.Value));
